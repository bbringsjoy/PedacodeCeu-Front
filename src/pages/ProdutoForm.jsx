...
import { criarProduto, editarProduto, listarProdutos, listarCategorias } from "../services/api";
...
export default function ProdutoForm() {
  ...
  useEffect(() => {
    carregarCategorias();
    if (editando && id) carregarProduto(Number(id));
  }, []);
  ...
  async function carregarProduto(produtoId) {
    try {
      ...
      const produto = res.dados.find((p) => p.id === produtoId);
      if (produto) {
  ...
  async function handleSubmit() {
    ...
    setCarregando(true);
    try {
      const payload = { nome, descricao, preco: Number(preco), categoriaId: Number(categoriaId) };
      if (editando && id) {
    ...
    <div className="pagina-crud" style={{ maxWidth: 500 }}>
      ...
      <div className="perfil-form">
        ...
        <div style={{ display: "flex", gap: 12 }}>
          <button className="botao botao--secundario" style={{ width: "auto" }} onClick={() => navigate("/produtos")}>
            Cancelar
          </button>
